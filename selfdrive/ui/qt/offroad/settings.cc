...
CarrotPanel::CarrotPanel(QWidget* parent, int mode) : QWidget(parent) {
  ...
  latLongToggles->addItem(new CValueControl("AdjustCurveOffset", "弯道偏移调整(0)cm", "弯道中路径相对车道中心的偏移量(厘米)。\n正值：向弯道内侧偏移（内切，减少离心力）\n负值：向弯道外侧偏移（外扩，增加安全距离）\n仅弯道上生效: 弯道速度≤50km/h 达到满额, 至200km/h 线性衰减为0; 叠加后的总偏移被夹紧在±40cm内。修正弯道居中偏差用。", -500, 500, 5));
  ...
  // === 丢目标缓冲（独立模块，默认关=零影响）===（已置顶：功能菜单最上端 · 2026-09-22）
  //     说明（中文）：雷达对小目标(电动车/摩托)会间歇性丢点。实测一次 48 分钟行程里"持续≥1.5秒的
  //     真丢失"共 89 次、平均持续 12.9 秒；lead 一丢，MPC 就失去跟随约束 ⇒ 目标速度回到巡航设定
  //     ⇒ 车立刻加速前冲(实测丢失期间最高车速平均 +8.2km/h、p90 +25.2、最大 +48)。开启本功能后，
  //     丢失瞬间用"丢失前最后一次观测"做运动学外推，虚构一个"虚拟前车"继续约束 MPC，直到真前车
  //     回来或缓冲超时。仅在近距离跟车时丢失才生效(高速巡航丢目标不干预)，且虚拟前车只会越推越近。
  featToggles->addItem(sectionHeader("丢目标缓冲辅助"));
  featToggles->addItem(new CValueControl("LeadBufferEnable", "丢目标缓冲(0)", "0=关闭(默认,零影响); 1=开启: 雷达丢失前车时, 用丢失前最后一次观测外推出一个\"虚拟前车\"继续约束纵向MPC, 防止丢目标瞬间失去跟随约束而加速前冲(实测丢失期间最高车速平均+8.2km/h)。仅近距离跟车时丢失才生效, 高速巡航丢目标不干预。", 0, 1, 1));
  featToggles->addItem(new CValueControl("LeadBufferTime", "缓冲时长(25)x0.1s", "丢目标后维持\"虚拟前车\"的最长时间(x0.1秒)。调大=更保守(丢失后更久不加速, 但前车真走了会跟得犹豫); 调小=更快恢复真实逻辑。默认25即2.5秒, 范围5~60(0.5~6.0秒)。需先开启\"丢目标缓冲\"。", 5, 60, 5));
  featToggles->addItem(new CValueControl("LeadBufferMaxDist", "缓冲最大距离(40)m", "仅当\"丢失前的前车距离\"小于本值时才启动缓冲(米)。调大=更多场景触发缓冲(含较远跟车); 调小=只在很近跟车丢失时才缓冲。默认40m, 范围10~80。需先开启\"丢目标缓冲\"。", 10, 80, 5));
  // === 静止障碍横向避让 v2（独立开关，默认关=零影响原逻辑；无视车道线让位，最多借一条邻车道）===（原置顶；按"新增功能置顶"规则让位给更新的分类）
  featToggles->addItem(sectionHeader("静止障碍避让辅助"));
  featToggles->addItem(new CValueControl("AvoidObstacle", "静止障碍避让(0)", "行驶中遇到前方静止或极慢的障碍(路边停车/抛锚车/故障车/行人)时, 朝远离障碍的一侧让位; 让位后与障碍的车对车横向间距必须达到[安全间距]才算有效, 做不到就横向一点不让并通知纵向减速; 允许越过本车道线、最多借入一条邻车道, 让位后车身外沿仍留在邻车道外线与道路边缘之内并留出余量; 正前方障碍是否绕行由[车流队列判据]决定; 0:关闭(完全不影响原逻辑), 1:开启", 0, 1, 1));
  ...
  featToggles->addItem(new CValueControl("AvoidOffsetLimit", "让位上限(250)cm", "单侧最多让出多少横向距离, 单位cm; 越大能让开的场景越多(实测绕开正前方障碍需约240~255cm), 越小越保守; 实际让位还会被[邻车道外线/道路边缘]与[安全间距]进一步夹紧; 开启[车流队列判据=2 正前方绕行]时建议调至280以上", 50, 400, 10));
  ...
  // === 路沿额外余量（护栏/隔离带/路肩是硬边界，比车道线多留的余量；默认30cm=0.30m）===
  //     说明（中文）：贴着护栏/隔离带/路肩那一侧，比普通车道线额外多留的横向余量；越大离硬边界越远越安全，但可用走廊更窄（更易判让不开）；越小越好绕但离护栏更近；可调范围 0~100 cm。
  featToggles->addItem(new CValueControl("AvoidEdgeExtra", "路沿余量附加(30)cm", "贴护栏/隔离带/路肩那一侧比车道线额外多留的余量, 单位cm; 越大离硬边界越远越安全但可用走廊更窄(更易判让不开), 越小越好绕但离护栏更近; 默认30cm", 0, 100, 5));
  // === 让位速率上限（横向让位每秒最多横移多少米；默认15=1.5m/s）===
  //     说明（中文）：让位量变化率上限，即每秒最多横移多少米；越大绕开越迅猛但更突兀、乘客更晃，越小越柔但可能来不及被判让不开；可调范围 5~30（即 0.5~3.0 m/s）。
  featToggles->addItem(new CValueControl("AvoidMaxShiftRate", "让位速率(15=1.5)m/s", "横向让位的变化率上限(每秒最多横移多少米); 越大绕开越迅猛但更突兀乘客更晃, 越小越柔但可能来不及被判让不开; 默认15即1.5m/s, 范围5~30(0.5~3.0m/s)", 5, 30, 1));
  // === 正前方判定带宽（区分"停车等待的车"与"偏侧停靠的静止车"；默认60cm=0.60m）===
  //     说明（中文）：本参数只负责**定义"算不算正前方"**这一件事——障碍横向偏移**小于**本值
  //     即判"挡在车道正中央（正前方）"，**大于**则判"偏侧停靠"。
  //     ★ 作用随[车流队列判据]而变（2026-09-22 用户指定三档=2）：
  //       · 判据=0/1：正前方 ⇒ 一律交纵向跟停、绝不横向绕行（本参数越小=越多车被判正前方=越保守）
  //       · 判据=2  ：正前方**也绕**（跨道绕开），本参数转而决定"多窄才算它真的挡在正中心"——
  //                  调小=只有几乎压线的才算正前方（偏侧车走偏侧分支）；调大=更宽的范围按正前方处理。
  //     两条分支都绕，差别只在走哪条让位计算路径，故本参数对"绕不绕"的影响已大幅减弱。
  featToggles->addItem(new CValueControl("AvoidCenterBand", "正前方判定带宽(60)cm", "障碍横向偏移小于本值即判[挡在车道正中央(正前方)], 大于则判[偏侧停靠]; 此参数只负责这条归属判定, [绕不绕]由上方[车流队列判据]决定(判据=2时正前方也跨道绕开); 默认60cm, 范围20~150", 20, 150, 5));
  // === 车流队列判据（三档；默认2=正前方也绕）===
  //     0=关闭: 只看横向偏置决定是否绕行(排队/车流也可能乱绕, 不推荐)
  //     1=启用: 障碍前方还有别的车 ⇒ 视为车流交纵向跟停; 正前方目标一律交纵向(不绕)
  //     2=启用+正前方也绕(默认, 用户指定): 正前方的车/行人也跨道绕开(简单粗暴);
  //       等灯/排队的防护靠[车流判据](前方还有车⇒交纵向)+[起手速度门槛10km/h]双兜底
  featToggles->addItem(new CValueControl("AvoidQueueCheck", "车流队列判据(2)", "0=关闭: 只看横向偏置决定是否绕行(排队时也可能乱绕,不推荐); 1=启用: 障碍前方还有别的车视为车流交纵向跟停, 正前方目标一律交纵向不绕; 2=启用+正前方也绕: 正前方的车/行人也跨道绕开(简单粗暴); 等灯排队靠[前方还有车⇒交纵向]+[低速10km/h不起手]双兜底; 建议[让位上限]≥280cm", 0, 2, 1));
