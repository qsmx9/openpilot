...
CarrotPanel::CarrotPanel(QWidget* parent, int mode) : QWidget(parent) {
  ...
  //     说明（中文）：障碍横向偏移**小于**本值时，视为"挡在车道正中央"（等红灯 / 排队 / 正前方跟车），
  //     一律交给纵向跟停、绝不做横向绕行；**大于**本值才算"偏侧停靠的静止车辆"，才允许评估横移绕行。
  //     调大=更保守（更多车被当作前方车而跟停、更少绕行）；调小=更积极（偏得很少的车也会尝试绕行）。
  featToggles->addItem(new CValueControl("AvoidCenterBand", "正前方判定带宽(60)cm", "障碍横向偏移小于本值时视为挡在车道正中央(等红灯/排队/正前方跟车), 交纵向跟停不绕行; 大于本值才算偏侧停靠的静止车辆, 才允许横移绕行; 调大更保守(少绕行), 调小更积极(更多绕行); 默认60cm, 范围20~150", 20, 150, 5));
  // === 车流队列判据（障碍前方还有别的车时视为车流，不绕行；默认1=开启）===
  //     说明（中文）：障碍虽偏侧，但若它前方还有别的车（车流 / 排队缓行），说明它是"车流里的一辆"，
  //     不是"孤立停靠的静止车辆"，此时交纵向跟停、不做横移绕行（避免在排队/堵车时乱打方向）。
  featToggles->addItem(new CValueControl("AvoidQueueCheck", "车流队列判据(1)", "1=启用: 障碍前方若还有别的车(车流/排队缓行), 视为车流里的一辆, 交纵向跟停、不做横移绕行(避免排队/堵车时乱打方向); 0=关闭: 只看横向偏置决定是否绕行", 0, 1, 1));
